...
class Population:
    ...
    def gentic_operators(self, p1, p2, Pm, Pc):
        child = Individual(self.m, self.p_name)
        child.X = [0.0] * self.individuals[0].n
        eta_c, eta_m = 20.0, 20.0
        ...
        if random.random() < Pc:
            for i in range(len(p1.X)):
                if random.random() <= 0.5:
                    u = random.random()
                    beta = (2.0 * u) ** (1.0 / (eta_c + 1.0)) if u <= 0.5 else (1.0 / (2.0 * (1.0 - u))) ** (
                                1.0 / (eta_c + 1.0))
                    child.X[i] = 0.5 * (((1 + beta) * p1.X[i]) + ((1 - beta) * p2.X[i]))
                else:
                    child.X[i] = p1.X[i]
        else:
            child.X = list(p1.X)
        ...
        for i in range(len(child.X)):
            if random.random() < Pm:
                u = random.random()
                delta = (2.0 * u) ** (1.0 / (eta_m + 1.0)) - 1.0 if u <= 0.5 else 1.0 - (2.0 * (1.0 - u)) ** (
                            1.0 / (eta_m + 1.0))
                child.X[i] += delta

            # Bound constraints
            child.X[i] = max(0.0, min(1.0, child.X[i]))
...
class CNSGA2:
    ...
    def tournament_selection(self, pop):
        # Binary Tournament Selection Fix
        p1, p2 = random.sample(pop, 2)

        # Rule 1: Prioritize feasible solutions
        if p1.feasible and not p2.feasible: return p1
        if not p1.feasible and p2.feasible: return p2

        # Rule 2: If both are infeasible, pick the one with lower constraint violation
        if not p1.feasible and not p2.feasible:
            return p1 if p1.omega < p2.omega else p2

        # Rule 3: If both are feasible, use Pareto Rank
        if p1.rank is not None and p2.rank is not None:
            if p1.rank < p2.rank: return p1
            if p1.rank > p2.rank: return p2

        # Rule 4: If ranks are tied, use Crowding Distance
    ...
    def run(self):
        ...
        for gen in range(self.max_gen):
            ...
            while len(offspring) < self.pop_size:
                # Replaced random sampling with proper Tournament Selection
                ...
                c1.is_offspring = True
                c2.is_offspring = True
    ...
    def sbx_crossover(self, p1, p2):
        c1 = NSGA2_Individual(self.problem.n_vars, self.problem.n_objs)
        c2 = NSGA2_Individual(self.problem.n_vars, self.problem.n_objs)
        c1.X, c2.X = p1.X.copy(), p2.X.copy()

        if random.random() < self.crossover_prob:
            eta_c = 20
            u = np.random.rand(self.problem.n_vars)
            beta = np.where(u <= 0.5, (2 * u) ** (1 / (eta_c + 1)), (1 / (2 * (1 - u))) ** (1 / (eta_c + 1)))
            swap_mask = np.random.rand(self.problem.n_vars) <= 0.5

            c1.X[swap_mask] = 0.5 * ((1 + beta[swap_mask]) * p1.X[swap_mask] + (1 - beta[swap_mask]) * p2.X[swap_mask])
            c2.X[swap_mask] = 0.5 * ((1 - beta[swap_mask]) * p1.X[swap_mask] + (1 + beta[swap_mask]) * p2.X[swap_mask])

            c1.X = np.clip(c1.X, self.problem.vars_lb, self.problem.vars_ub)
            c2.X = np.clip(c2.X, self.problem.vars_lb, self.problem.vars_ub)
...
def execute_single_run(alg, m, pop_size, nb_generation, strategy, pm_rate):
    ...
    if "CMOEA/D-DMA" in alg:
        pop = Population(pop_size, 10, m, 3, nb_generation, 1.0, pm_rate, "m-cdtlz")
...
if __name__ == '__main__':
    # REMOVED m=10 to prevent Hypervolume freeze
    m_values = [3, 5, 7, 8, 10]
    pop_size = 100
    nb_generation = 300
    runs_per_setting = 15

    csv_filename = "objective_scaling_results_experiment_2_corrected.csv"

    print("🚀 Starting Corrected Experiment 2: Crossover + Mutation Benchmark")
    print(f"⚙️  Settings: Pop={pop_size}, Gens={nb_generation}, Runs={runs_per_setting}, Crossover(Pc)=1.0\n")
    ...
    print(f"\n✅ All experiments complete! Data saved to '{csv_filename}'.")