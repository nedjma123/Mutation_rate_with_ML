...
def active_learning_logger(
        ...
        exec_time, hypervolume, target_mutation_rate, filename="new_training_data.csv"
...
class AIBrain:
    ...
    def predict_rate(self, n_var, n_obj, n_constr, pop_size):
        ...
        for rate in test_rates:
            rows.append({
                ...
                'Number of Generations': 800, 'Mutation Rate': rate,
...
class Individual:
    ...
    def fitness(self, m):
        ...
        if "Welded Beam" in self.p_name:
            ...
            f1 = (0.6224 * x[0] * x[2] * x[3]) + (1.7781 * x[1] * (x[2] ** 2)) + (3.1661 * (x[0] ** 2) * x[3]) + (
                        19.84 * (x[0] ** 2) * x[2])
    ...
    def constraints_g(self, m):
        ...
        if "Welded Beam" in self.p_name:
            ...
            tau = math.sqrt(tau_prime ** 2 + 2 * tau_prime * (P * (L + l / 2) * R / J) * l / (2 * R) + (
                        P * (L + l / 2) * R / J) ** 2)
            ...
            self.g = [x[0] - 0.0193 * x[2], x[1] - 0.00954 * x[2],
                      (math.pi * x[2] ** 2 * x[3] + 4 / 3 * math.pi * x[2] ** 3) - 1296000, 240 - x[3]]
    ...
    def Update_Solution_Archive(self, y, ind, alpha):
        ...
        for i in range(len(d)):
            ...
            if self.individuals[k].feasible and y.feasible and (r < r1):
                ...
                viol_y = sum(
                    [abs(v) for item in y.g for v in (item if isinstance(item, list) else [item])] if y.g else [0])

                viol_xj = sum(
                    [abs(v) for item in self.individuals[k].g for v in (item if isinstance(item, list) else [item])] if
                    self.individuals[k].g else [0])
...
# 6. MAIN EXECUTION (Safe 5-Way Loop)

# ==========================================

# ==========================================
# 6. MAIN EXECUTION (Safe 5-Way Loop with GLOBAL NORMALIZATION)
# ==========================================
if __name__ == '__main__':
    ...
    for p_name in PROBLEMS:
        ...
        ai_pm_initial = AI.predict_rate(n_v, m, n_c, pop_size)
        ...
        for strategy_name, pm_rate in strategies:
            print(f"\n▶️ Running Strategy: {strategy_name}", flush=True)
        ...
        if len(all_feasible_f_vals_for_problem) > 0:
            ...
            print(f"\n   🌍 Global Bounds Found! Min: {np.round(global_min, 2)} | Max: {np.round(global_max, 2)}",
                  flush=True)
        else:
            print(f"\n   ⚠️ WARNING: No feasible solutions found by ANY strategy for {p_name}.", flush=True)
        ...
        for strategy_name, _ in strategies:
            ...
            for run_data in run_data_vault[strategy_name]:
                ...
                exec_time = run_data['exec_time']
                final_pm = run_data['final_pm']
                ...
                if len(f_vals) > 0 and global_min is not None:
                    # NORMALIZING AGAINST THE GLOBAL BOX
                ...
                if strategy_name == "AI-Adaptive":
                    active_learning_logger(
                        problem=p_name, problem_type="Constrained", algorithm="MOEA/D-DMA",
                        pop_size=pop_size, n_constr=n_c, n_obj=m, archive_size=pop_size,
                        mutation_type="Polynomial", crossover_rate=Pc_experiment, crossover_type="SBX",
                        n_gen=nb_generation, n_var=n_v, ref_point=[1.1] * m,
                        exec_time=exec_time, hypervolume=run_hv, target_mutation_rate=final_pm
                    )

            # --- Save CSV Results for Strategy ---
            mean_hv = np.mean(hv_list)
            std_hv = np.std(hv_list)
            avg_success = np.mean(success_count_list)
        ...
        # ---------------------------------------------------------
        # 🔬 PHASE 4: STATISTICAL COMPARISON (Mann-Whitney U)
        # ---------------------------------------------------------
        ...
        for baseline in baselines:
            ...
            if np.allclose(ai_data, baseline_data):
                ...
                try:
                    ...
                    print(f"      ⚠️ STATS CRASH vs {baseline:<20} | Error: {e}", flush=True)
                    p_value = 1.0
